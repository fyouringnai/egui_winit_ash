...
impl App {
    ...
    fn create_pipeline(&mut self) {
        ...
        let vert = include_bytes!("shader/triangle_vert.spv");
        let frag = include_bytes!("shader/triangle_frag.spv");
        ...
        let position_attribute_description = VertexInputAttributeDescription {
            location: 0,
            binding: 0,
        ...
        let vertex_input_info = PipelineVertexInputStateCreateInfo {
            ...
            p_vertex_attribute_descriptions: [position_attribute_description].as_ptr(),
            ..Default::default()
        };
    ...
    fn record_command_buffer(&mut self, index: usize) {
        ...
        unsafe {
            ...
            device.cmd_push_constants(
                ...
                0,
                cast_slice(&[self.vertex_colors[0]]),
            );
            ...
            device.cmd_push_constants(
                ...
                ShaderStageFlags::VERTEX,
                size_of::<Vec4>() as u32,
                cast_slice(&[self.vertex_colors[1]]),
            );
            ...
            device.cmd_push_constants(
                ...
                self.pipeline_layout,
                ShaderStageFlags::VERTEX,
                2 * size_of::<Vec4>() as u32,
                cast_slice(&[self.vertex_colors[2]]),
            );
    ...
    fn create_buffer(&mut self) {
        ...
        let get_type_index =
            |memory_requirements: MemoryRequirements, properties: MemoryPropertyFlags| -> u32 {
                ...
                (0..memory_type_count)
                    .find(|i| {
                        let is_type_supported =
                            memory_requirements.memory_type_bits & (1 << i) as u32 != 0;
                        let has_required_properties = memory_properties.memory_types[*i]
                            .property_flags
                            .contains(properties);
        ...
        let create_buffer = |size: DeviceSize,
         ...
         -> (Buffer, DeviceMemory) {
            ...
            let memory_type_index = get_type_index(memory_requirements, properties);
        ...
        let copy_buffer = |src: Buffer, dst: Buffer, size: DeviceSize| {
            let command_pool = self.command_pools[0];
            ...
            unsafe {
                ...
                device
                    .queue_submit(self.queues[0], &[submit_info], Fence::null())
                    .expect("Failed to submit queue");

                device
                    .queue_wait_idle(self.queues[0])
        ...
        let vertex_size = size_of::<Vec3>() * vertices.len();
        let index_size = size_of::<u32>() * indices.len();

        let (staging_buffer, staging_memory) = create_buffer(
            vertex_size as DeviceSize,
        ...
        unsafe {
            copy_nonoverlapping(vertices.as_ptr(), memory_ptr.cast(), vertex_size);
        ...
        copy_buffer(staging_buffer, vertex_buffer, vertex_size as DeviceSize);
        ...
        unsafe {
            copy_nonoverlapping(indices.as_ptr(), memory_ptr.cast(), index_size);
            device.unmap_memory(staging_memory);
        }
        ...
        copy_buffer(staging_buffer, index_buffer, index_size as DeviceSize);
    ...
    fn window_event(
        ...
        match event {
            WindowEvent::RedrawRequested => {
                ...
                gui.run(|egui_ctx| {
                    containers::Window::new("Hello Triangle").show(egui_ctx, |ui| {
                        ui.heading("Color");
                        ui.label("Left Bottom");
                        ui.add(Slider::new(&mut self.vertex_colors[0].x, 0.0..=1.0).text("R"));
                        ui.add(Slider::new(&mut self.vertex_colors[0].y, 0.0..=1.0).text("G"));
                        ui.add(Slider::new(&mut self.vertex_colors[0].z, 0.0..=1.0).text("B"));
                        ui.add_space(10.0);
                        ui.label("Right Bottom");
                        ui.add(Slider::new(&mut self.vertex_colors[1].x, 0.0..=1.0).text("R"));
                        ui.add(Slider::new(&mut self.vertex_colors[1].y, 0.0..=1.0).text("G"));
                        ui.add(Slider::new(&mut self.vertex_colors[1].z, 0.0..=1.0).text("B"));
                        ui.add_space(10.0);
                        ui.label("Top");
                        ui.add(Slider::new(&mut self.vertex_colors[2].x, 0.0..=1.0).text("R"));
                        ui.add(Slider::new(&mut self.vertex_colors[2].y, 0.0..=1.0).text("G"));
                        ui.add(Slider::new(&mut self.vertex_colors[2].z, 0.0..=1.0).text("B"));
                    });
                });
